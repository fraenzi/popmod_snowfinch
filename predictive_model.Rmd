...
```{r nrfeldglings, echo=FALSE, fig.cap="Number of fledglings in relation to the date when the first egg of the brood hatched. The regression line is from a linear mixed model with hatching date as predictor and year as random factor. The 95% CI is given as dotted lines."}
...
legend(2, 1.4, lwd=2, lty=c(1,3), legend=c("1st brood", "2nd brood"), bty="n", cex=0.8)

plot(juvS, juvS.lambda, ylim=yminmax, type="l", lwd=2, lty=3, las=1, xlab="Female survival")
abline(h=mean(trend_currentknowledge), col="grey")
lines(adS, adS.lambda, lwd=2)
legend(0.4, 0.8, lwd=2, lty=c(3,1), legend=c("first year", "adult"), bty="n", cex=0.8)
...
Knowing exactly the average number of broods per pair (i.e. the proportions of non-breeders, single and double brooded pairs) and the number of fledglings per brood has similar effects on the precision of the predicted population trends (Figure \@ref(fig:vispopdiffknowl)). It reduces the uncertainty of the predicted population size in 10 years from a range of `r round(popsizes_lwr[10])`- `r round(popsizes_upr[10])` to ranges of `r round(popsizesnrbrknown_lwr[10])`- `r round(popsizesnrbrknown_upr[10])` if the average number of broods is exactly known and `r round(popsizesnfknown_lwr[10])`- `r round(popsizesnfknown_upr[10])` if the average number of fledglings is exactly known. We get a remarkable increase in precision of the predicted population size if we know survival exactly `r round(popsizesSknown_lwr[10])`- `r round(popsizesSknown_upr[10])`.
...
polygon(c(1:nyears, rev(1:nyears)), 
        ...
        col=rgb(0.5,1,0.5,0.2))

polygon(c(1:nyears, rev(1:nyears)), 
        c(popsizesSknown_lwr, rev(popsizesSknown_upr)), border=NA,
        col=rgb(0.7,0.1,0.7,0.2))

lines(1:nyears, popsizesnrbrknown_mean, lwd=2, col=rgb(1,1,0.2))
lines(1:nyears, popsizesnfknown_mean, lwd=2, col=rgb(0.5,1,0.5))
lines(1:nyears, popsizesSknown_mean, lwd=2, col=rgb(0.7,0.1,0.7))
legend(1, 8400, lwd=2, col=c("brown", "yellow", "lightgreen", "darkviolet"), legend=c("current knowledge", "number of broods known", "number of fledglings known", "survival known"), bty="n", cex=0.8)
```
